...
function appendBubble(author, text, meta = '') {
  if (author === 'You') {
    ...
    avatar.innerHTML = '<img src="/static/jovian/images/logo.png">'; // you can change to emoji or icon
    ...
    title.textContent = meta || 'TBI Agent';
    ...
    subtitle.textContent = '...'; // static label, tweak if you like
...
async function startStream(e) {
  ...
  try {
    ...
    await streamResponseToEvents(resp, (event) => {
      ...
      const node = event.node || 'message';

      if (node === 'general_message' && !event?.complete) {
